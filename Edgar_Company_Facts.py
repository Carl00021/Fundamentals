...
def get_cik(stock):
    ...
    cik= tickers_cik.loc[stock].iloc[0]
    return cik
...
def get_financial_data(cik):
    ...
    shares_list = ['CommonStockSharesOutstanding','WeightedAverageNumberOfSharesOutstandingBasic', 'WeightedAverageNumberOfDilutedSharesOutstanding','StockholdersEquityNoteStockSplitConversionRatio1']
    list_check = [item in df_quarterly.columns for item in shares_list]
    #NetIncomeLossAvailableToCommonStockholdersBasic, NetIncomeLossAvailableToCommonStockholdersDiluted
    ...
    if list_check[0] == True:
        ...
        bso = df_annual['WeightedAverageNumberOfSharesOutstandingBasic'].combine_first(df_quarterly['WeightedAverageNumberOfSharesOutstandingBasic'])
        df_quarterly['WeightedAverageNumberOfSharesOutstandingBasic'] = bso
        df_annual.loc[df_annual['WeightedAverageNumberOfSharesOutstandingBasic'].notnull(), 'WeightedAverageNumberOfSharesOutstandingBasic'] = bso
    ...
    if list_check[2] ==  True:
        ...
        if list_check[3] == True:
            ...
            df_quarterly['AdjustedDSO'].iloc[0] = df_quarterly['WeightedAverageNumberOfDilutedSharesOutstanding'][1] *stock_split[-1]
            ...
            for i in range(1,len(df_quarterly.index)):      
                df_quarterly['AdjustedDSO'].iloc[i] = min((df_quarterly['WeightedAverageNumberOfDilutedSharesOutstanding'][i] * stock_split), key=lambda num: abs(num - df_quarterly['AdjustedDSO'][i-1]))
        else:
            df_quarterly['AdjustedDSO'] = df_quarterly[shares_list[2]]
    ...
    revenue_list = ['RevenueFromContractWithCustomerExcludingAssessedTax','TotalRevenuesAndOtherIncome','SalesRevenueNet','Revenues','SalesRevenueGoodsNet','RevenuesNetOfInterestExpense','SalesRevenueServicesNet','CoalProductsAndServicesRevenue']
    ...
    netincome_list = ['NetIncomeLoss','ProfitLoss']
    cogs_list = ['COGS_implied','CostOfGoodsAndServicesSold','CostOfGoodsSold','CostOfRevenue','CostOfServices','CostOfGoodsSoldExcludingDepreciationDepletionAndAmortization']
    ...
    sga_list = ['SellingGeneralAndAdministrativeExpense','S+G&A','SellingAndAdministrativeExpensesExcludingGoodwillAndIntangibleAssetImpairmentCharges']
    interest_list = ['InterestExpense','InterestIncomeExpenseNet','InterestIncomeExpenseNonoperatingNet','InterestAndOtherNonoperatingExpenses']
    ...
    cash_list = ['CashAndCashEquivalentsAtCarryingValue','ShortTermInvestments']
    stdebt_list = ['DebtCurrent','ShortTermBorrowings']
    ltdebtm1_list=['LongTermDebtMaturitiesRepaymentsOfPrincipalInNextTwelveMonths']
    ltdebt_list = ['LongTermDebt','LongTermDebtFairValue','LongTermDebtNoncurrent','LongTermDebtAndCapitalLeaseObligations']
    ...
    merge_clean_line_item(df_annual, df_annual_clean, stdebt_list, 'ShortTermDebt')
    ...
    merge_clean_line_item(df_quarterly, df_quarterly_clean, stdebt_list, 'ShortTermDebt')
    ...
    #Exceptions for companies that do not report certain lines (e.g. operating income)
    if df_quarterly_clean['OperatingIncome'].isnull().all().all(): #Some companies do not report operating income, this uses EBIT instead.
        df_quarterly_clean['OperatingIncome'] = df_quarterly_clean['NetIncome'] + df_quarterly_clean['Tax']+ df_quarterly_clean['Interest'].fillna(0)
        df_annual_clean['OperatingIncome'] = df_annual_clean['NetIncome'] + df_annual_clean['Tax']+ df_annual_clean['Interest'].fillna(0)
    if  df_quarterly_clean['GrossProfit'].isnull().all().all():
        df_quarterly_clean['GrossProfit'] = df_quarterly_clean['Revenues'] - df_quarterly_clean['COGS']
    
    #New Calculations -------------------------
    df_quarterly_clean['Adjusted DSO'] = df_quarterly['AdjustedDSO']
    df_quarterly_clean['Growth Capex'] = df_quarterly_clean['Capex'] - df_quarterly_clean['D&A']
    df_quarterly_clean['Maint Capex'] = df_quarterly_clean['D&A']
    df_quarterly_clean['Growth Investments'] = df_quarterly_clean['Growth Capex'].fillna(0) + df_quarterly_clean['R&DExpense'].fillna(0) + df_quarterly_clean['SG&AExpense'].fillna(0) #Idea is growth investments
    df_quarterly_clean['LongTermDebt'] = df_quarterly_clean['LongTermDebt'].fillna(0)
    df_quarterly_clean['FCFE'] = df_quarterly_clean['CashFromOperations'] - df_quarterly_clean['Capex']
    df_quarterly_clean['FCFEexSBC'] = df_quarterly_clean['CashFromOperations'] - df_quarterly_clean['Capex'] - df_quarterly_clean['SBC'].fillna(0)
    ...
    df_quarterly_clean['ROIIC_V2'] = (df_quarterly_clean['NOPAT'] - df_quarterly_clean['NOPAT'].shift(4)) / (df_quarterly_clean['Growth Investments']) #Idea is growth investments
    df_quarterly_clean['ROA'] = df_quarterly_clean['NetIncome'] / df_quarterly_clean['TotalAssets']
    ...
    df_quarterly_clean['NetDebt'] = df_quarterly_clean['ShortTermDebt'].fillna(0) + df_quarterly_clean['LongTermDebt'].fillna(0) - df_quarterly_clean['Cash'].fillna(0)
    ...
    df_ltm_clean['ROIC'] = df_ltm_clean['NOPAT'] / df_ltm_clean['InvestedCapital']
    df_ltm_clean['ROIIC'] = (df_ltm_clean['NOPAT'] - df_ltm_clean['NOPAT'].shift(4)) / (df_ltm_clean['InvestedCapital'] - df_ltm_clean['InvestedCapital'].shift(4))
    df_ltm_clean['ROIIC_V2'] = (df_ltm_clean['NOPAT'] - df_ltm_clean['NOPAT'].shift(4)) / (df_ltm_clean['Growth Investments']) #Idea is growth investments
    df_ltm_clean['ROA'] = df_ltm_clean['NetIncome'] / df_ltm_clean['TotalAssets']
    df_ltm_clean['ROE'] = df_ltm_clean['NetIncome'] / df_ltm_clean['TotalStockholderEquity']
    df_ltm_clean['ROCE'] = df_ltm_clean['EBIT'] / (df_ltm_clean['TotalAssets'] - df_ltm_clean['CurrentLiabilities'])
    df_ltm_clean['NCAV'] = df_ltm_clean['CurrentAssets'] - df_ltm_clean['TotalLiabilities']
